...
class App extends Component {
  ...
  UNSAFE_componentWillMount() {
    ...
    formRef.on('child_added', snapshot => {
      const {title, author, isbn, publisher, date, date2, comment} =
          snapshot.val();
  ...
  render() {
    return (
        <div>{
          this.state.alert &&
          <div className = {`alert alert-${this.state.alertData.type}`} role =
               'alert'><div className = 'container'>{
              this.state.alertData.message}</div>
        </div>
        }<div className = 'container' style = {{ padding: `40px 0px` }}>
            <div className = 'row'><div className = 'col-sm-4'><h2>Book Vault<
                /h2>
              <form onSubmit={this.sendMessage.bind(this)} ref='bookVault' >
                <div className='form-group'>
                  <label htmlFor='title'>Title</label>
            <input type = 'text' className = 'form-control' id =
                 'title' placeholder = 'The title of the book' ref =
            ...
            <input type = 'text' className = 'form-control' id =
                 'publisher' placeholder = 'The Publisher of the book' ref =
             {
        ...
        <input type = 'text' classname = 'form-control' id =
             'date' placeholder =
                 'The date in which you have started the book' ref =
         { date => this.inputDate = date } />
                </div>
        <div className = 'form-group'><label htmlFor = 'date2'>Date of Finish<
            /label>
        ...
        <textarea type = 'text' className = 'form-control' id = 'comment' rows =
             '3' ref = {comment => this.textComment = comment}>
        </textarea>
                </div>
        ...
        <h6 className = 'card-subtitle mb-2 text-muted'>{form.author}<
            /h6>
                        <h6 className='card-subtitle mb-2 text-muted'>{form.publisher}</h6>
        <p className = 'card-text'>{form.isbn}<
            /p>
